 ...
 *
 * Real File System Tools for OpenAgent
...
/**
 * Create real file tool - Actually writes files to disk
 */
export const createRealFileTool = tool(
  async (input: { file_path: string; content: string }) => {
    ...
    try {
      ...
      return `✅ Successfully created real file: ${file_path}`;
    } catch (error) {
      return `❌ Failed to create file '${file_path}': ${error instanceof Error ? error.message : String(error)}`;
    }
  },
  {
    name: "create_real_file",
    description: "Create a real file on the filesystem (not virtual)",
    schema: z.object({
...
/**
 * Read real file tool - Reads actual files from disk
 */
export const readRealFileTool = tool(
  ...
  {
    name: "read_real_file",
    description: "Read a real file from the filesystem (not virtual)",
    schema: z.object({
...
/**
 * Update real file tool - Modifies actual files on disk
 */
export const updateRealFileTool = tool(
  async (input: {
    file_path: string;
    old_text: string;
    new_text: string;
    ...
    try {
      ...
      return `✅ Successfully updated real file: ${file_path}`;
    } catch (error) {
      return `❌ Failed to update file '${file_path}': ${error instanceof Error ? error.message : String(error)}`;
    }
  },
  {
    name: "update_real_file",
    description: "Update a real file on the filesystem by replacing text",
    schema: z.object({
...
/**
 * List real directory tool
 */
export const listRealDirectoryTool = tool(
  ...
  },
  {
    ...
    description: "List contents of a real directory on the filesystem",
    schema: z.object({
...
/**
 * Create real directory tool
 */
export const createRealDirectoryTool = tool(
  ...
  {
    name: "create_real_directory",
    description: "Create a real directory on the filesystem",
    schema: z.object({
